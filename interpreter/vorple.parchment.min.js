...
h.html(h.html().replace(/^.*\n/,""));var k=a("span:last",h);var m=[];if (k.html()) { a("input",k).appendTo(h);m= k.html().split("\n");}if(1<m.length){(g.prompt=m.pop(),k.html(m.join("\n")+"\n"))} else if (k.html){(g.prompt=k.html(),k.remove())}}else g.prompt="";g.$turn=h;return g},xa=function(b,d){if(!b)return!1;switch(d.turn.type){case "error":b.classes+="parserError ";b.previousCommand.classes+="parserError ";b.content.classes+="transient ";b.meta=!0;break;case "undo":case "meta":return""!==a.trim(a(b.content.text).text())&&vorple.notify.show(b.content.text),
